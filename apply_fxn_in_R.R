#Apply family of functions in R ----
  ...
  m1
  a_m1 <- apply(m1, 2, sum)
  a_m1 #here 2 means column ie column sum 
  class(a_m1)
  str(a_m1)
  a_m2 <- apply(m1, 1, mean)
  a_m2  #here 1 means row sum
  class(a_m2)
  str(a_m2)
  ...
  # Note: 
  # sapply() input=list ,data fram ,vextor  returns output in vector/matrix whereas 
  # lapply() input=list ,data fram ,vextor retuns output in list.