...
t1   #dnn command comments the words  

table(mtcars$cyl)
table(mtcars$gear)
table(mtcars$cyl,mtcars$gear)  #count Gear vs Cyl
#1 cars with gear4 and cyl=3
...
# ~ this is tilda sign ie cyl currosponds to gear
...
# sd =standard deviation  =(sq of variance) and variance is (value - mean) 
#prop.table-------
?prop.table
#This fxn basically calculates proportion of values 
t1
prop.table(t1)
#Cyl4, Gear3 : 1 out of 32 = 1/32 ie 1/32=0.03125

prop.table(t1, margin=1)  #sum of each row=1
#here 1/11 ie  11 is sum of row 
...
#sweep----
sweep(m1, MARGIN = 1, STATS = c(2,3,4), FUN="+" ) #colwise ADDDITION
